...
class Configurator():
	...
	def _configure_module_recursive(self, module):
		...
		self.graph.printer.print_progress("-" * self.depth  + ">" + module.get_name())
	...
	def tag(self, tags):
		self.active_module.set_tags(package_utils._split_tags(tags))
		return self

	def type(self, type_value):
		self.active_module.set_type(type_value)
		return self
	...
	def _sync_config(self, graph, abs_path_to_ini_file, override_config):
		
		if override_config:
			self.graph.printer.print_status(f"CONFIG: {abs_path_to_ini_file} (overriten!)")
			self._save_config_ini(graph, abs_path_to_ini_file, ["Regenerated by user."])
			return None

		self.graph.printer.print_status(f"CONFIG: {abs_path_to_ini_file}")
